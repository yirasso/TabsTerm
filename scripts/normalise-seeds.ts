 ...
 *
 * Safe to re-run without `--from`: normaliseGrid leaves content that is already
...
import { normaliseGrid, regrid } from "../src/lib/tab/grid.ts";
...
const rewritten = source.replace(/content: `([\s\S]*?)`,/g, (whole, content: string) => {
  const next = fromWidth === null ? normaliseGrid(content) : regrid(content, fromWidth);
